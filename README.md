...
并且所有的线程都可以访问到进程的全局区(全局变量和静态变量),堆区(可以任意访问到new的内存地址空间)

![](./image/线程栈.jpg)

但对于栈来讲,每个线程都是独立的..并且,每个线程都有自己独立的堆区(跟进程的堆区有不一样,可能会开辟独立的内存地址空间)

![](./image/栈修改.jpg)

但需要注意的是,尽管栈是线程私有数据,但是栈没有添加任何保护,我们是可以修改属于任何一个线程的栈区的

***线程和线程之间是独立的堆栈,但没有限制他的读和写***
...
用锁保护的代码是无法并发的,只能串行执行,这样就解决了线程间的共享内存的问题
...
我们用分页模型来管理物理内存,首先我们把物理内存空间分为4KB大小页
